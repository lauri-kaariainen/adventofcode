...
const handleInputToObjects = (input: string): Bag[] =>
    input
        ...
        .map((line: string): Bag => {
            const [_,
                color,
            ] = line.match(/(\w+ \w+)/i) as Array<string>
            return {
                color,
                innerBags: (line.match(/(\d+ \w+ \w+)+/gi) || [])
                    .map((bagStr: string) => ({
                        amount: parseInt(bagStr.split(" ")[0]),
                        color: bagStr.split(" ").slice(1).join(" ")
                    }))
            } as Bag
        })
        ...
        const matchedItems =
            list.filter((bag: Bag) =>
                bag.innerBags
                    ...
                    .filter((innerBagColor: string) => searchedOccurrences.includes(innerBagColor)).length !== 0)
        return iterateAndCountOccurrences(
            //take out those bags that have searchedOccurrence color bag inside 
            list.filter((bag: Bag) => !matchedItems.map((bag: Bag) => bag.color).includes(bag.color)),
            matchedItems.map((bag: Bag) => bag.color),
