...
const checkIfValid = (str: string, minimumValidKeys: string[], runStrictTest: boolean = false) => {
    ...
    return minimumValidKeys
        ...
        !runStrictTest ? true :
        true &&
        betweenOrEqual(parseInt(passportObj.byr), 1920, 2002) &&
        betweenOrEqual(parseInt(passportObj.iyr), 2010, 2020) &&
        betweenOrEqual(parseInt(passportObj.eyr), 2020, 2030) &&
        passportObj.hgt.match(/^\d+[a-z]{2}$/i) &&
        (passportObj.hgt.toLowerCase().match(/[a-z]+/)![0] === "cm" &&
            betweenOrEqual(parseInt(passportObj.hgt.match(/\d+/)![0]), 150, 193)
            ||
            (passportObj.hgt.toLowerCase().match(/[a-z]+/)![0] === "in" &&
                betweenOrEqual(parseInt(passportObj.hgt.match(/\d+/)![0]), 59, 76))) &&
        passportObj.hcl.match(/^#\w{6}$/i) &&
...
log("test a:",
    ...
    testInputArr.filter((line: string) => checkIfValid(line, VALIDSFORA)).length

)

log("a:",
    inputArr.filter((line: string) => checkIfValid(line, VALIDSFORA))
        .length
)


log("test b:",
    getValidTestInput().filter((line: string) => checkIfValid(line, VALIDSFORA, true)).length,
    getInvalidTestInput().filter((line: string) => checkIfValid(line, VALIDSFORA, true)).length

)

log("b:",
    inputArr.filter((line: string) => checkIfValid(line, VALIDSFORA, true)).length
)
...
function handleInputStr(str: string) {
    return str.split(/\n\s+\n/).map(e => e.trim().replace(/\n\s+/g, " "))
}


function getTESTInput(): string[] {
    return handleInputStr(`ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
function getValidTestInput(): string[] {
    return handleInputStr(`pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
    hcl:#623a2f
...
function getInvalidTestInput(): string[] {
    return handleInputStr(`eyr:1972 cid:100
