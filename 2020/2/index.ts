...
const betweenOrEqual = (number: number, min: number, max: number): boolean =>
    number >= min && number <= max


const turnInputIntoObjects = (inputArr: string[]): InputObject[] =>
    inputArr.map((line: string) => {
        const matches = line.match(/(\d+)-(\d+) (\w): (\w+)/);
        return (
            {
                number1: parseInt(matches![1]),
                number2: parseInt(matches![2]),
                letter: matches![3],
                code: matches![4]
            } as InputObject
        )
...
const checkIfValid = (inputObj: InputObject): boolean =>
    betweenOrEqual(
        inputObj.code.split("").filter(letter =>
            letter === inputObj.letter).length,
        inputObj.number1,
        inputObj.number2)

log("test a:",
    turnInputIntoObjects(testInputArr)
        .filter((elem: InputObject) => checkIfValid(elem)).length
)
...
const checkIfValidPartTwo = (inputObj: InputObject): boolean =>
    (inputObj.code[inputObj.number1 - 1] == inputObj.letter ||
        inputObj.code[inputObj.number2 - 1] == inputObj.letter) && (
        inputObj.code[inputObj.number1 - 1] !== inputObj.code[inputObj.number2 - 1]
    )
...
function getInput(): string[] {
    return (
        ...
        2-3 k: ftkkdc
        2-9 g: jpxcgggzgsgngrhght`.split("\n")
    );
}