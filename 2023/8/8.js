...
const input =
{
    instructions: getInput().split("\n")[0].trim().split(""),
    network:
        getInput()
            .split(/\n\n/)[1]
            .split("\n")
            .map(singleLine => ({
                address: singleLine.split(" = (")[0],
                L: singleLine.split(" = (")[1].split(", ")[0],
                R: singleLine.split(" = (")[1].split(", ")[1].slice(0, 3)
            }))
            .reduce((acc, next) => {
                acc[next.address] = { L: next.L, R: next.R };
                return acc;
            }, {})
}
...
const traverseNetwork = ({ instructions, network }, route = []) => {
    ...
    return () => traverseNetwork(
        ...
        route.concat(network[route.slice(-1)[0]][nextDirection])
