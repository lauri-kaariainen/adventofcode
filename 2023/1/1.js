 ...
 * @type {string[]} 
 */
const numbers = Object.keys(DIGITMAP)
/**
 * @type {string}
 */
...
const changedInput =
    getInput()
        .replaceAll(
            new RegExp(numbers.join("|"), 'g'),
            /**
             * 
             * @param {string} num 
             * @returns {string}
             */
            (num) => {/*console.log(num);*/ return DIGITMAP[num]})//DIGITMAP[num])
console.log(changedInput)
console.log("B: WRONG RESULT:",
    "" +
    changedInput
        .split("\n")
        .map(
            line => line.match(/\d/)[0] + line.split("").reverse().join("").match(/\d/)[0])
        .filter(pair => (console.log(pair), true))
        .reduceRight((acc, next) => acc + parseInt(next), 0)
)
...
function getInput() {
    return `ckmb52fldxkseven3fkjgcbzmnr7
