...
"use strict";
/**
 * @typedef {{"red":number, "green":number,"blue":number}} Cubes
 */

/**
 * @typedef {Cubes} Limits
 */
...
/**@type {number[]} */
const mutatingFailedIDs = []

/**@type {Cubes[]} */
const mutatingMinimumCubesPerGame = []
...
getInput()
    ...
    .map(
         ...
         */
        (line, lineNum) => {
            mutatingMinimumCubesPerGame.push({
            ...
            [1]
                ...
                /**
                 * 
                 * @param {string} oneReach 
                 * @returns {string[]}
                */
                .flatMap(oneReach => oneReach.split(", "))
                .forEach(oneCubeString => {
                    const [amount, color] = oneCubeString.split(" ");
                    //keep the largest value present for a certain cube value
                    mutatingCubes[oneCubeString.split(" ")[1]] =
                        Math.max(
                            mutatingCubes[oneCubeString.split(" ")[1]],
                            parseInt(oneCubeString.split(" ")[0]))
                    if (amount > limits[color]) {
                        // console.log("failed:", lineNum + 1)
                        mutatingFailedIDs.push(lineNum + 1)
                    }
                    mutatingMinimumCubesPerGame[lineNum][color] =
                        Math.max(
                            mutatingMinimumCubesPerGame[lineNum][color],
                            parseInt(amount)
                        )
 ...
 * @type {number[]}
 */
const uniqueFails = Array.from(new Set(mutatingFailedIDs))

console.log("A:",
    allIds.filter(id => !uniqueFails.includes(id)).reduce((a, b) => a + b, 0)
)
console.log("B:",
    mutatingMinimumCubesPerGame
        .map(
            /**
             * @type {Cubes}
             */
            obj => obj.red * obj.green * obj.blue)
        .reduce((a, b) => a + b, 0)
 ...
 */
function getInput() {
    return `Game 1: 3 blue, 2 green, 6 red; 17 green, 4 red, 8 blue; 2 red, 1 green, 10 blue; 1 blue, 5 green
